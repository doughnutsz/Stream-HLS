...
for i in {0..1} ; do
  ...
  if [ -f $kernel/mlir/intermediates/${kernel}_default.py ]; then
    echo "{ \"Sequential Cycles\": " > results_streamhls/${rpt_name}.json
    python $kernel/mlir/intermediates/${kernel}_default.py >> results_streamhls/${rpt_name}.json
    echo "," >> results_streamhls/${rpt_name}.json
    echo "\"Parallel Cycles\": " >> results_streamhls/${rpt_name}.json
    python $kernel/mlir/intermediates/${kernel}_default.py >> results_streamhls/${rpt_name}.json
    echo "," >> results_streamhls/${rpt_name}.json
    echo "\"Combined Cycles\": " >> results_streamhls/${rpt_name}.json
    python $kernel/mlir/intermediates/${kernel}_default.py >> results_streamhls/${rpt_name}.json
    echo "}" >> results_streamhls/${rpt_name}.json
  else
